...
def train_one_epoch(model, dataloader, optimizer, device, epoch_idx=None, num_epochs=None):
    """
    ...
    loop_desc = f"Train Epoch [{epoch_idx}/{num_epochs}]" if (epoch_idx and num_epochs) else "Training"
...
def evaluate(model, dataloader, device, epoch_idx=None, num_epochs=None):
    ...
    loop_desc = f"Val Epoch [{epoch_idx}/{num_epochs}]" if (epoch_idx and num_epochs) else "Validation"
    pbar = tqdm(dataloader, desc=loop_desc, leave=False)
...
def main():
    ...
    train_dataset = KittiVLOdomDataset(
        root_dir=root_dir,
        sequences_to_load=train_sequences,
        annotated=True
    )
    val_dataset = KittiVLOdomDataset(
        root_dir=root_dir,
        sequences_to_load=val_sequences,
        annotated=True
    )
    ...
    # ------------------------------------------------
    model = SimpleStereoPoseNet(pretrained=False).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train_one_epoch(
            ...
            epoch_idx=epoch+1, num_epochs=num_epochs
        )
